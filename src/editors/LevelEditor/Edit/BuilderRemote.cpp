...
BOOL SceneBuilder::BuildMesh(const Fmatrix& parent, CEditableObject* object, CEditableMesh* mesh, int sect_num,
    b_vertex* verts, int& vert_cnt, int& vert_it, b_face* faces, int& face_cnt, int& face_it, u32* smgroups,
...
BOOL SceneBuilder::BuildObject(CSceneObject* obj)
{
    ...
    for (EditMeshIt M = O->FirstMesh(); M != O->LastMesh(); M++)
    {
        ...
        if (!BuildMesh(T, O, *M, sect_num, l_verts, l_vert_cnt, l_vert_it, l_faces, l_face_cnt, l_face_it, l_smgroups,
                obj->_Transform()))
            return FALSE;
        // fill DI vertices
...
BOOL SceneBuilder::BuildMUObject(CSceneObject* obj)
{
    ...
    if (-1 == model_idx || m_save_as_object)
    {
        ...
        for (EditMeshIt MESH = O->FirstMesh(); MESH != O->LastMesh(); ++MESH)
            if (!BuildMesh(T, O, *MESH, sect_num, M.m_pVertices, M.m_iVertexCount, vert_it, M.m_pFaces, M.m_iFaceCount,
                    face_it, M.m_smgroups, obj->_Transform()))
                return FALSE;
...
BOOL SceneBuilder::BuildLight(CLight* e)
{
    ...
    Fvector dir;
    dir.setHP(e->PRotation.y, e->PRotation.x);
    ...
    default: THROW2("Invalid light type."); return FALSE;
    }
}
...
{
    return BuildMaterial(surf->_ShaderName(), surf->_ShaderXRLCName(), surf->_Texture(),
        ((surf->_FVF() & D3DFVF_TEXCOUNT_MASK) >> D3DFVF_TEXCOUNT_SHIFT), sector_num, allow_draft);
}

int SceneBuilder::BuildMaterial(
    LPCSTR esh_name, LPCSTR csh_name, LPCSTR tx_name, u32 tx_cnt, int sector_num, bool allow_draft)
{
    b_material mtl;
    ZeroMemory(&mtl, sizeof(mtl));
    VERIFY(sector_num >= 0);
...
BOOL SceneBuilder::ParseStaticObjects(ObjectList& lst, LPCSTR prefix, bool b_selected_only)
{
    ...
    for (ObjectIt _F = lst.begin(); _F != lst.end(); _F++)
    {
        ...
        switch ((*_F)->ClassID)
        {
        case OBJCLASS_LIGHT: bResult = BuildLight((CLight*)(*_F)); break;
        case OBJCLASS_GLOW: bResult = BuildGlow((CGlow*)(*_F)); break;
...
BOOL SceneBuilder::CompileStatic(bool b_selected_only)
{
    ...
    if (bResult && !l_lods.empty())
    {
        ...
        if (1 ==
            ImageLib.CreateMergedTexture(2, images, merged_image, 512, 2048, 64, 2048, offsets, scales, rotated, remap))
        {
