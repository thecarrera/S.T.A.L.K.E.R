...
#include "editors/ECore/Editor/EThumbnail.h"
#include "SceneSummaryInfo.h"
#include "Scene.h"
#include "SceneObject.h"
#include "editors/ECore/Editor/ui_main.h"
#include "UI_LevelMain.h"
#include "editors/ECore/Editor/Library.h"

static SSceneSummary s_summary;

xr_token summary_texture_type_tokens[] = {{"Base", SSceneSummary::sttBase}, {"Implicit", SSceneSummary::sttImplicit},
    {"DO", SSceneSummary::sttDO}, {"Glow", SSceneSummary::sttGlow}, {"LOD", SSceneSummary::sttLOD}, {0, 0}};
...
void SSceneSummary::STextureInfo::FillProp(PropItemVec& items, LPCSTR main_pref, u32& mem_use)
{
    if (file_name.size())
    {
        ...
        PropValue* V = 0;
        V = PHelper().CreateChoose(items, PrepareKey(pref.c_str(), "Texture"), &file_name, smTexture);
        V->Owner()->Enable(FALSE);
        PHelper().CreateCaption(items, PrepareKey(pref.c_str(), "Format"), info.FormatString());
        PHelper().CreateCaption(items, PrepareKey(pref.c_str(), "Size"),
            shared_str().printf("%d x %d x %s", info.width, info.height, info.HasAlpha() ? "32b" : "24b"));
        PHelper().CreateCaption(
            items, PrepareKey(pref.c_str(), "Memory Usage"), shared_str().printf("%d Kb", iFloor(tex_mem / 1024)));
        PHelper().CreateCaption(
            items, PrepareKey(pref.c_str(), "Effective Area"), shared_str().printf("%3.2f m^2", effective_area));
        PHelper().CreateCaption(items, PrepareKey(pref.c_str(), "Pixel Density"),
            shared_str().printf("%3.2f p/m", _sqrt((pixel_area * info.width * info.height) / effective_area)));
        /*
        //. убрал из-за кол-ва > 4096
                AnsiString tmp 		= "on demand";
                for (objinf_map_it o_it=objects.begin(); o_it!=objects.end(); o_it++){
                    tmp += AnsiString().sprintf("%s%s[%d*%3.2f]",tmp.Length()?";
        ":"",o_it->first.c_str(),o_it->second.ref_count,o_it->second.area);
                }
        ...
        */
        if (info.flags.is_any(STextureParams::flDiffuseDetail | STextureParams::flBumpDetail))
        {
            if (0 != info.detail_name.size())
            {
                ...
                V->Owner()->Enable(FALSE);
                PHelper().CreateCaption(
                    items, PrepareKey(pref.c_str(), "Detail Scale"), shared_str().printf("%3.2f", info.detail_scale));
            }
            else
            {
        ...
        ButtonValue* B = PHelper().CreateButton(
            items, PrepareKey(pref.c_str(), "Highlight Texture"), "Select,Density =,Density +,Clear", 0);
...
void SSceneSummary::STextureInfo::Export(IWriter* F, u32& mem_use)
{
    ...
    for (objinf_map_it o_it = objects.begin(); o_it != objects.end(); o_it++)
    {
        tmp2 += AnsiString().sprintf("%s%s[%d*%3.2f]", tmp2.Length() ? "; " : "", o_it->first.c_str(),
            o_it->second.ref_count, o_it->second.area);
    }
    int tex_mem = info.MemoryUsage(*file_name);
    mem_use += tex_mem;
    tmp.sprintf(mask, *file_name, info.FormatString(), info.width, info.height, info.HasAlpha() ? "present" : "absent",
        iFloor(tex_mem / 1024), effective_area, _sqrt((pixel_area * info.width * info.height) / effective_area),
        tmp2.c_str(), *info.detail_name, info.detail_scale, *info.bump_name);
...
void SSceneSummary::FillProp(PropItemVec& items)
{
    ...
    PHelper().CreateCaption(
        items, "Geometry\\Bounding\\Min", shared_str().printf("{%3.2f, %3.2f, %3.2f}", VPUSH(bbox.min)));
    PHelper().CreateCaption(
        items, "Geometry\\Bounding\\Max", shared_str().printf("{%3.2f, %3.2f, %3.2f}", VPUSH(bbox.max)));
    ...
    u32 total_mem_usage = 0;
    ButtonValue* BB = PHelper().CreateButton(
        items, "Textures\\Highlight Textures\\Command", "Pixel Density,Clear", ButtonValue::flFirstOnly);
    ...
    for (PDVecIt pd_it = pm_colors.begin(); pd_it != pm_colors.end(); pd_it++)
    {
        ...
        FloatValue* V =
            PHelper().CreateFloat(items, PrepareKey(tmp, "Weight (p/m)").c_str(), &pd_it->pm, 0, 1000000, 1, 0);
...
{
    if (I->section_exist("summary_info") && I->line_exist("summary_info", "pm_count") &&
        (5 == I->r_u32("summary_info", "pm_count")))
    {
        for (u32 idx = 0; idx < s_summary.pm_colors.size(); ++idx)
        {
            string128 tmp;
