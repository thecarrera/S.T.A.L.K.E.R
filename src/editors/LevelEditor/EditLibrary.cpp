...
#include "Scene.h"
#include "Builder.h"
#include "editors/ECore/Engine/Texture.h"
#include "BottomBar.h"
...
//---------------------------------------------------------------------------
__fastcall TfrmEditLibrary::TfrmEditLibrary(TComponent* Owner) : TForm(Owner)
{
    ...
    m_Items = TItemList::CreateForm("Objects", paItems, alClient,
        TItemList::ilMultiSelect | TItemList::ilEditMenu | TItemList::ilDragAllowed | TItemList::ilFolderStore);
...
bool TfrmEditLibrary::GenerateLOD(ListItemsVec& props, bool bHighQuality)
{
    ...
    for (; it != it_e; ++it)
    {
        ...
        if (O && O->IsMUStatic())
        {
            ...
            ImageLib.CreateLODTexture(O, tex_name.c_str(), LOD_IMAGE_SIZE, LOD_IMAGE_SIZE, LOD_SAMPLE_COUNT,
                O->Version(), bHighQuality ? 4 /*7*/ : 1);
            O->OnDeviceDestroy();
    ...
    /*
        ...
        for( ;it!=it_e; ++it)
        {
            ...
            if (O && O->IsMUStatic())
            {
                ...
                tex_name 				= xr_string("lod_")+tmp;
                tex_name 				= ImageLib.UpdateFileName(tex_name);
                ImageLib.CreateLODTexture(O,
       tex_name.c_str(),LOD_IMAGE_SIZE,LOD_IMAGE_SIZE,LOD_SAMPLE_COUNT,O->Version(),bHighQuality?7:1);
                O->OnDeviceDestroy	();
                O->m_objectFlags.set(CEditableObject::eoUsingLOD,bLod);
                ELog.Msg			(mtInformation,"LOD for object '%s' successfully created.",O->GetName());
            }else{
...
void TfrmEditLibrary::MakeLOD(bool bHighQuality)
{
    ...
    /*
        ...
        {
            LockForm();
            int res 				= ELog.DlgMsg(mtConfirmation,TMsgDlgButtons() << mbYes << mbNo << mbCancel,"Do you
    want to select multiple objects?");
            if (res!=mrCancel)
            {
                if (res==mrYes)
                {
                    ...
                    {
                        if (mrYes==ELog.DlgMsg(mtConfirmation,TMsgDlgButtons() << mbYes << mbNo,"Are you sure to
    generate LOD for all object in this folder?"))
                        {
                            int iLODcnt = 0;
                ...
                if (FS.exist(save_nm.c_str()))
                    if (mrNo ==
                        ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                            "Object '%s' already exist. Owerwrite it?", nm.c_str()))
                    {
                        xr_delete(O);
                        break;
                    }
