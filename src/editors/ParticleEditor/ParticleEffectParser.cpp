...
void pEAvoid(float magnitude, float epsilon, float look_ahead, PDomainEnum dtype, float a0, float a1, float a2,
    float a3, float a4, float a5, float a6, float a7, float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pEBounce(float friction, float resilience, float cutoff, PDomainEnum dtype, float a0, float a1, float a2, float a3,
    float a4, float a5, float a6, float a7, float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pEDamping(float damping_x, float damping_y, float damping_z, float vlow, float vhigh)
{
...
void pEExplosion(float center_x, float center_y, float center_z, float velocity, float magnitude, float stdev,
    float epsilon, float age, BOOL allow_translate, BOOL allow_rotate)
{
...
void pEJet(float center_x, float center_y, float center_z, float magnitude, float epsilon, float max_radius,
    BOOL allow_translate, BOOL allow_rotate)
{
...
void pEOrbitLine(float p_x, float p_y, float p_z, float axis_x, float axis_y, float axis_z, float magnitude,
    float epsilon, float max_radius, BOOL allow_translate, BOOL allow_rotate)
{
...
void pEOrbitPoint(float center_x, float center_y, float center_z, float magnitude, float epsilon, float max_radius,
    BOOL allow_translate, BOOL allow_rotate)
{
...
void pERandomAccel(PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5, float a6, float a7,
    float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pERandomDisplace(PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5, float a6, float a7,
    float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pERandomVelocity(PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5, float a6, float a7,
    float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pESink(BOOL kill_inside, PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5, float a6,
    float a7, float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pESinkVelocity(BOOL kill_inside, PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5,
...
void pESource(float particle_rate, PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5,
    float a6, float a7, float a8, BOOL allow_translate, BOOL allow_rotate)
{
    _ParticleState& _ps = _GetPState();
...
void pETargetColor(float color_x, float color_y, float color_z, float alpha, float scale)
{
...
void pETargetSize(float size_x, float size_y, float size_z, float scale_x, float scale_y, float scale_z)
{
...
void pETargetRotateD(float scale, PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5,
...
void pETargetVelocityD(float scale, PDomainEnum dtype, float a0, float a1, float a2, float a3, float a4, float a5,
    float a6, float a7, float a8, BOOL allow_translate, BOOL allow_rotate)
{
...
void pEVortex(float center_x, float center_y, float center_z, float axis_x, float axis_y, float axis_z, float magnitude,
    float epsilon, float max_radius, BOOL allow_translate, BOOL allow_rotate)
{
...
xr_token pddomain_token[] = {
    {"PDPoint", PDPoint}, {"PDLine", PDLine}, {"PDTriangle", PDTriangle}, {"PDPlane", PDPlane}, {"PDBox", PDBox},
    {"PDSphere", PDSphere}, {"PDCylinder", PDCylinder}, {"PDCone", PDCone}, {"PDBlob", PDBlob}, {"PDDisc", PDDisc},
    {"PDRectangle", PDRectangle}, {0, 0},
};
xr_token type_token[] = {
    {"PDomainEnum", ptDomain}, {"BOOL", ptBOOL}, {"float", ptFloat}, {"int", ptInt}, {"WORD", ptWORD}, {"u16", ptWORD},
    {"DWORD", ptDWORD}, {"u32", ptDWORD}, {"LPSTR", ptString}, {"LPCSTR", ptString}, {0, 0},
};
...
union var
{
    ...
    var& operator=(WORD _w)
    {
        w = _w;
        return *this;
    }

    var& operator=(DWORD _o)
    {
        o = _o;
        return *this;
    }

    var& operator=(string128& _s)
    {
        strcpy(s, _s);
        return *this;
    }
...
IC
    ...
    bool
    get_domain_type(LPCSTR src, var& val)
{
...
#define P15 P14, p(14)
        ...
        if (command == "pColor")
            pColor(P4);
        else if (command == "pColorD")
            pColorD(P11);
        else if (command == "pVelocity")
            pVelocity(P3);
        else if (command == "pVelocityD")
            pVelocityD(P10);
        else if (command == "pVertexB")
            pVertexB(P3);
        else if (command == "pVertexBD")
            pVertexBD(P10);
        else if (command == "pVertexBTracks")
            pVertexBTracks(P1);
        else if (command == "pSize")
            pSize(P3);
        else if (command == "pSizeD")
            pSizeD(P11);
        else if (command == "pRotate")
            pRotate(P3);
        else if (command == "pRotateD")
            pRotateD(P10);
        else if (command == "pStartingAge")
            pStartingAge(P2);
        else if (command == "pSetMaxParticles")
            pSetMaxParticles(P1);
        ...
        else if (command == "pAvoid")
            pEAvoid(P15);
        else if (command == "pBounce")
            pEBounce(P15);
        else if (command == "pCopyVertexB")
            pECopyVertexB(P1);
        else if (command == "pDamping")
            pEDamping(P5);
        else if (command == "pExplosion")
            pEExplosion(P10);
        else if (command == "pFollow")
            pEFollow(P3);
        else if (command == "pGravitate")
            pEGravitate(P3);
        else if (command == "pGravity")
            pEGravity(P5);
        else if (command == "pJet")
            pEJet(P8);
        else if (command == "pKillOld")
            pEKillOld(P2);
        else if (command == "pMatchVelocity")
            pEMatchVelocity(P3);
        else if (command == "pMove")
            pEMove();
        else if (command == "pOrbitLine")
            pEOrbitLine(P11);
        else if (command == "pOrbitPoint")
            pEOrbitPoint(P8);
        else if (command == "pRandomAccel")
            pERandomAccel(P12);
        else if (command == "pRandomDisplace")
            pERandomDisplace(P12);
        else if (command == "pRandomVelocity")
            pERandomVelocity(P12);
        else if (command == "pRestore")
            pERestore(P1);
        else if (command == "pSink")
            pESink(P13);
        else if (command == "pSinkVelocity")
            pESinkVelocity(P13);
        else if (command == "pSource")
            pESource(P13);
        else if (command == "pSpeedLimit")
            pESpeedLimit(P2);
        else if (command == "pTargetColor")
            pETargetColor(P5);
        else if (command == "pTargetSize")
            pETargetSize(P6);
        else if (command == "pTargetRotate")
            pETargetRotate(P4);
        else if (command == "pTargetRotateD")
            pETargetRotateD(P11);
        else if (command == "pTargetVelocity")
            pETargetVelocity(P6);
        else if (command == "pTargetVelocityD")
            pETargetVelocityD(P13);
        else if (command == "pVortex")
            pEVortex(P11);
...
static LPCSTR PActionCommands[] = {
    ...
    "pExplosion(float center_x, float center_y, float center_z, float velocity, float magnitude, float stdev, float "
    "epsilon=EPS_L, float age=0.0f, BOOL allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    ...
    "pJet(float center_x, float center_y, float center_z, float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT, BOOL allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pKillOld(float age_limit, BOOL kill_less_than=FALSE);",
    "pMatchVelocity(float magnitude=1.0f, float epsilon=EPS_L, float max_radius=P_MAXFLOAT);", "pMove();",
    "pOrbitLine(float p_x, float p_y, float p_z, float axis_x, float axis_y, float axis_z, float magnitude=1.0f, float "
    "epsilon=EPS_L, float max_radius=P_MAXFLOAT, BOOL allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pOrbitPoint(float center_x, float center_y, float center_z, float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT, BOOL allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    ...
    "pTargetSize(float size_x, float size_y, float size_z, float scale_x=0.0f, float scale_y=0.0f, float "
    "scale_z=0.0f);",
    "pTargetRotate(float rot_x, float rot_y, float rot_z, float scale=0.0f);",
    "pTargetRotateD(float scale, PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, float a3=0.0f, float "
...
void CPEDef::Compile()
{
    ...
    */
    ...
    try
    {
        ...
        // save effect data
        ParticleEffect* pe = _GetEffectPtr(effect_handle);
        R_ASSERT(pe);
