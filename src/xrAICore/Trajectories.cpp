...
#include "xrGame/Level.h" // XXX AI: introduce interface or remove dependency
#include "xrPhysics/IPHWorld.h"
#include "Debug/ai_debug_variables.h"
#include "xrCore/_vector3d_ext.h"

static void trajectory_get_position(Fvector &result, const Fvector &start_position,
    const Fvector &velocity, const Fvector &gravity, const float &time)
{
    // result = start_position + velocity*t + gravity*t^2/2
    result.mad(start_position, velocity, time).mad(gravity, _sqr(time)*0.5f);
}

inline static float trajectory_max_error_time(float t0, float t1)
{ return (t1+t0)*0.5f; }

static float trajectory_pick_error(float low, float high,
    const Fvector &position, const Fvector &velocity, const Fvector &gravity)
{
...
static float trajectory_select_pick_time(const float &start_low, float high,
    const Fvector &start, const Fvector &velocity, const Fvector &gravity, const float &epsilon)
{
    float low = start_low;
    float check_time = high;
    float time_epsilon = 0.1f/velocity.magnitude();
...
static bool trajectory_check_collision(float low, float high,
    const Fvector &position, const Fvector &velocity, const Fvector &gravity,
    IGameObject *self_object, IGameObject *ignored_object, Fvector &collide_position,
    collide::rq_results &temp_rq_results, Fvector box_size,
    xr_vector<trajectory_pick> *out_trajectory_picks, xr_vector<Fvector> *out_collide_tris)
{
    Fvector start;
    trajectory_get_position(start, position, velocity, gravity, low);
    Fvector target;
    ...
    if (distance<0.01f)
        return false;
    start_to_target.mul(1.0f/distance);
...
bool trajectory_intersects_geometry(float trajectory_time,
    const Fvector &trajectory_start, const Fvector &trajectory_end, const Fvector &trajectory_velocity,
    Fvector &collide_position, IGameObject *self_object, IGameObject *ignored_object,
    collide::rq_results &temp_rq_results, xr_vector<trajectory_pick> *out_trajectory_picks,
    xr_vector<Fvector> *out_collide_tris, const Fvector &box_size)
{
...
#endif
    const Fvector gravity = Fvector().set(0, -physics_world()->Gravity(), 0);
    ...
    while (true)
    {
        ...
        if (!trajectory_check_collision(low, time, trajectory_start, trajectory_velocity,
            gravity, self_object, ignored_object, collide_position, temp_rq_results,
